...
def choose(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    e_paragraphs = [x for x in paragraphs if select(x)]
    if len(e_paragraphs) > k:
        return e_paragraphs[k]
    else:
        return ''
...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def helper(sentense):
        A = split(lower(remove_punctuation(sentense)))
        K = 0
        while K < len(topic):
            for x in A:
                if x == topic[K]:
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    asc = min(valid_words, key=lambda w:diff_function(user_word, w, limit))
    if diff_function(asc, user_word, limit) > limit:
        return user_word
    else:
        return asc
    # END PROBLEM 5
...
def pawssible_patches(start, goal, limit):
    ...
    elif start == 0 or goal == 0: # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
